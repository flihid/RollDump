  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card divide-y">
        {(q.data?.items || []).map((row: any) => (
          <div key={row.id} className="p-3 text-sm">
            <div className="flex items-center gap-2">
              ...
              <span className="text-ink-500 text-xs">{new Date(row.createdAt).toLocaleString('id-ID')}</span>
            </div>
        ...
        {q.data?.items?.length === 0 && <div className="p-6 text-center text-sm text-ink-500">Belum ada log.</div>}
      </div>
    </div>
  );
}
